  ...
  return function(err) {
    ...
    pop.forEach(function(subOptions) {
      ...
      if (typeof subOptions.model === 'string') {
        ...
        subSchema = _this.model(schema.caster.options.ref).schema._getSchema(subOptions._originalPath);
        ...
        subSchema = _this.model(schema.options.ref).schema._getSchema(subOptions._originalPath);
        if (subSchema && subSchema.options && subSchema.options.ref) {
          subOptions.model = _this.model(subSchema.options.ref);
        } else if (subSchema && subSchema.caster && subSchema.caster.options &&
          subSchema.caster.options.ref) {
...
function populate(model, docs, options, cb) {
  ...
  promise = new Promise(function (err, vals, mod, assignmentOpts) {
    if (err) return cb(err);
...
function getModelsMapForPopulate(model, docs, options) {
  ...
  for (i = 0; i < len; i++) {
    ...
    var ret = convertTo_id(utils.getValue(options.path, doc));
    var id = String(utils.getValue('_id', doc));
