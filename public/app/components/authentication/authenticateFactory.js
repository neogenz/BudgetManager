appBudgetManager.factory('authenticateWebApi', function ($http, $localStorage, $q) {
    ...
    function _getUserFromLocalStorageToken() {
        ...
        var user = {};
        if (!app.helpers.isUndefinedOrNull(token)) {
            var encoded = token.split('.')[1];
            user = JSON.parse(urlBase64Decode(encoded));
        } else {
            app.helpers.logInfo('authenticateFactory.js', '_getUserFromLocalStorageToken()', 'The token in local storage is null or undefined.');
        }
        return user;
    }
    ...
    function _signup(beanUser) {
        var def = $q.defer();
        var requestOptions = app.helpers.buildPostRequestOptToCallThisUrl(app.budgetManager.endpoints['nodeEndpoint'] + '/signup', beanUser);
    ...
    function _signin(formData) {
        var def = $q.defer();
        var requestOptions = app.helpers.buildPostRequestOptToCallThisUrl(app.budgetManager.endpoints['nodeEndpoint'] + '/signin', formData);
        var promise = $http(requestOptions);
        promise.success(function (data) {
            $localStorage.token = data.token;
            def.resolve(data);
    ...
    function _checkIfThisPasswordIsGood(password) {
        var def = null,
            ...
            promise = null;
        def = $q.defer();
        requestOptions = app.helpers.buildGetRequestOptToCallThisUrl(app.budgetManager.endpoints['nodeEndpoint'] + '/checkPassword/' + password);
    ...
    function _changePassword(formData) {
        ...
        def = $q.defer();
        def = $q.defer();
        requestOptions = app.helpers.buildPostRequestOptToCallThisUrl(app.budgetManager.endpoints['nodeEndpoint'] + '/changePassword', formData);
        promise = $http(requestOptions);
        promise.success(function (data) {
            $localStorage.token = data.token;
            def.resolve(data);
