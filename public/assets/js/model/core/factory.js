            ...
            if (!_.isUndefined(json[realKey]) && !_.isObject(json[realKey])) {
                ...
                if (this._schema[key].type === app.beans.type.ARRAY_OBJECT) {
                    ...
                    for (var i = 0; i < json[realKey].length; i++) {
                        this[key].push(app.beans.factory.createBean(this._schema[key].contentObject.beanName, json[realKey][i]));
                    }
                } else {
                    this[key] = app.beans.factory.createBean(this._schema[key].beanName, json[realKey]);
                }
            }
        }
    };
